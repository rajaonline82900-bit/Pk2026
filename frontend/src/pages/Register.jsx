...
import { Eye, EyeOff, Gift, User, Phone, Lock, UserPlus, ShieldCheck, MessageCircle } from "lucide-react";
...
export default function Register() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (name.length < 2) return toast.error("Apna naam daalein");
    if (mobile.length !== 10) return toast.error("Mobile 10 digit ka hona chahiye");
    if (password.length < 4) return toast.error("Password kam se kam 4 characters");
    setLoading(true);
    try {
      await register(mobile, name, password, refCode || undefined);
      toast.success("Welcome to Raja Khaiwal!");
      navigate("/");
      ...
      <div className="w-full max-w-sm relative z-10">
        ...
        <div className="rounded-3xl p-4 border border-yellow-400/25 shadow-[0_20px_60px_-15px_rgba(251,191,36,0.25)]"
          ...
          <div className="grid grid-cols-2 gap-1.5 p-1.5 rounded-full bg-blue-950/70 border border-yellow-400/15 mb-5">
            <button type="button" onClick={() => navigate("/login")} className="h-11 rounded-full font-bold text-sm tracking-wider text-slate-300 hover:text-yellow-300 transition"
          ...
          <form onSubmit={submit} className="space-y-3.5" data-testid="register-form" autoComplete="off">
            ...
            <FieldWrap icon={<User className="w-4 h-4" />}>
              <input data-testid="register-name-input" value={name}
            ...
            <FieldWrap icon={<Lock className="w-4 h-4" />}>
              <input data-testid="register-mpin-input" type={showPw ? "text" : "password"}
            ...
            <FieldWrap icon={<Gift className="w-4 h-4" />}>
              <input data-testid="register-refcode-input" value={refCode}
                onChange={(e) => setRefCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 12))}
                placeholder="E.G. RK240869"
                autoComplete="off"
                className="w-full h-12 bg-transparent text-yellow-50 placeholder-slate-500 pl-11 pr-3 rounded-2xl outline-none font-mono tracking-widest" />
            </FieldWrap>
            {refCode && <div className="text-xs text-emerald-400 pl-1 -mt-1">✓ Aap refer honge — {refCode}</div>}

            <button type="submit" disabled={loading} data-testid="register-submit-btn"
              className="w-full h-14 rounded-2xl font-black text-base tracking-wider text-slate-900 flex items-center justify-center gap-2 disabled:opacity-60 mt-2"
              ...
              {loading ? "…" : (<><span>CREATE ACCOUNT</span><UserPlus className="w-5 h-5" /></>)}
            </button>
          </form>

          <div className="mt-5 text-center text-sm text-slate-300">
            पहले से खाता है? <Link to="/login" className="text-yellow-300 font-bold underline underline-offset-4 decoration-yellow-400/70" data-testid="link-to-login">लॉगिन करें</Link>
          </div>
        </div>
